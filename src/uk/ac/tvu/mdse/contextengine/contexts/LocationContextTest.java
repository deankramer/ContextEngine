...
//	    }
	...
	protected ArrayList<String> isNearby(Location locale){
		...
		String[] nearbys = new String[this.valuesSets.get(0).locationsSet.size()];
		float [] distances = new float[this.valuesSets.get(0).locationsSet.size()];
		...
		for(int i = 0; i<this.valuesSets.get(0).locationsSet.size();i++){// locationSet.entrySet()){
			if(locale.distanceTo(valuesSets.get(0).locationsSet.get(i).location) <= distancebetween){
				//nearbys.add(entry.getKey());
				nearbys[i] = valuesSets.get(0).locationsSet.get(i).identifier;
				distances[i] = locale.distanceTo(valuesSets.get(0).locationsSet.get(i).location);
	...
	public void sendNotification(ArrayList<String> nearbys) {
		if (D) Log.d(LOG_TAG, "sendNotification(nearbys)");
		...
		if (D) Log.d(LOG_TAG, "sendNotification(ContextValues).contextInformation:" +nearbys.get(2));
		if (D) Log.d(LOG_TAG, "sendNotification(ContextValues).contextInformation:" +nearbys.get(3));
		intent.putExtra(CONTEXT_APPLICATION_KEY, valuesSets.get(0).getKeysList());
		if (D) Log.d(LOG_TAG, "sendNotification(ContextValues)-keylist0:"+valuesSets.get(0).getKeysList());
		try {
